...
void insert_free(void *bp){
    ...
    if(NEXT_FREEP(start_free)==end_free){
        ...
        while(freep!=start_free && GET_ALLOC(HDRP(freep))){
            freep = PREV_BLKP(bp);
        }
        if(freep==bp){
            NEXT_FREEP(start_free) = bp;
    ...
    // // 첫블록
    // printf("첫블록\n");
    // PUT(PRVP(bp), (char*)start_free);   // get으로 호출하면 나오는 값
    // PUT(NXTP(start_free), (char*)bp);
    // PUT(NXTP(bp), (char*)end_free);
    // PUT(PRVP(end_free), (char*)bp);
    // printf("bp_get : %d, %d\n", GET(PRVP(bp)), GET(NXTP(bp)));
    // printf("bp의 prvp, nxtp : %d, %d\n", PRVP(bp), NXTP(bp));
    // printf("start : %d, end : %d\n", GET(start_free), GET(end_free));
    // printf("넣고나서 end의 prev에 : %d\n", GET(PRVP(end_free)));
