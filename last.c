...
int mm_init(void)
{
    ...
    PUT_P(heap_listp+(2*WSIZE), NULL);                // 프롤로그 prev
    PUT_P(heap_listp+(3*WSIZE), NULL);                // 프롤로그 next
    ...
    start_free = heap_listp;    // 프롤로그 header
...
void delete_free(void *bp){
    ...
    if(NEXT_FREEP(bp)){         // 다음 있을 때
        if(PREV_FREEP(bp)){         // 이전 있으면
            NEXT_FREEP(PREV_FREEP(bp)) = NEXT_FREEP(bp);
            PREV_FREEP(NEXT_FREEP(bp)) = PREV_FREEP(bp);
        }
        else{                       // 이전 없으면
            NEXT_FREEP(start_free) = NEXT_FREEP(bp);
...
void insert_free(void *bp){
    ...
    void * freep = bp;
    void * tmp = heap_listp;
    if(NEXT_FREEP(start_free)==NULL){
        NEXT_FREEP(start_free) = bp;
        PREV_FREEP(bp) = NULL;
        NEXT_FREEP(bp) = NULL;
    }
    else{
        while(tmp < bp && GET_ALLOC(HDRP(freep))){
            tmp = NEXT_BLKP(tmp);
            freep = PREV_BLKP(bp);
        }
        if(freep==bp){  // 이전블록
            NEXT_FREEP(start_free) = bp;
            PREV_FREEP(bp) = NULL;
            NEXT_FREEP(bp) = NEXT_FREEP(start_free);
        }
        else{
            if(NEXT_FREEP(freep)){ // 중간
                NEXT_FREEP(bp) = NEXT_FREEP(freep);
                PREV_FREEP(NEXT_FREEP(freep)) = bp;
                PREV_FREEP(bp) = freep;
                NEXT_FREEP(freep) = bp;
            }
            else{   // freep가 마지막
...
static void *find_fit(size_t asize){
    ...
    for(bp = start_free; bp != NULL; bp = NEXT_FREEP(bp)){
        printf("find_fit for 들어옴 %p\n", bp);
        if(bp==NEXT_FREEP(bp)) break;
...
// static void *next_fit(size_t asize){
//     printf("next_fit size : %d\n", asize);
//     void *bp;
//     if(NEXT_FREEP(start_free)){
        
//         for(bp=NEXT_BLKP(last_listp); bp!=NULL; bp=NEXT_FREEP(bp)){
//             if(asize <= GET_SIZE(HDRP(bp))){
//                 last_listp = bp;
//                 return bp;
//             }
